...
In an RFP system, where plasma resistivity is not null, there is a need for a mechanism to maintain the magnetic configuration balance over time otherwise diffusivity would flatten the toroidal magnetic field; in this way the magnetic energy would be dissipated out, and the RFP configuration would be lost.
...
% If the parallel component of the Ohm law is considered:
% \begin{equation}
%     E_\parallel = \frac{\Vec{E}\cdot\Vec{B}}{b} = \frac{E_\varphi B_\vartheta}{B} = \eta j_\parallel
% \end{equation}
% Since $E$

% The dynamo electric field is provided by a self-organization of the plasma velocity flows, through the term $\Vec{v} \times \Vec{B}$ in Ohm's law:
